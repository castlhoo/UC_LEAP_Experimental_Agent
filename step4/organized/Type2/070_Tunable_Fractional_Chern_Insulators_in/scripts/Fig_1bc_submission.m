...
xn0 = BG_cum*(1-delta)+(1+delta)*TG_cum;
yp0 = -BG_cum*(1-delta)+(1+delta)*TG_cum;

n0_density = 3*(xn0+0.9)*8.85e-12/1.6e-19/33.1e-9*10^-4*1e-12;
y_D = yp0 ./ 33.1*1000;
v = (xn0+0.95)/(8.75-0.95);
averaged_squid_signal = movmean(SQUID_DC_cum,smooth,1);
averaged_squid_signal= averaged_squid_signal - mean(averaged_squid_signal(end-300:end,:));
f = figure(1);
clf
...
pbaspect([max(max(xn0))-min(min(xn0)) max(max(yp0))-min(min(yp0)) 1])
